...
const ClaseLiderazgo = ({ clase }) => {
	const { id, module, part, videoLink, ppt, docxManual, docxLecture } =
		clase[0];
	...
	return (
		<>
			<Head>
				<meta
					name="description"
					content="Aprende sobre liderazgo cristiano con este curso gratuito ofrecido por el instructor Reynaldo Navedo"
				/>
				<meta name="autor" content="Reynaldo Navedo" />
				<meta
					name="keywords"
					content="blog cristiano, lecturas cristianas, liderazgo cristiano"
				/>
			</Head>
			<Layout
				pageTitle={`Taller de liderazgo cristiano - Módulo ${module} - Parte ${part}`}
			>
				...
				<section className="blog-one blog-one__home blog-one__grid">
					<div className="container">
						...
						<div className="mb-2">
							...
							<a href={docxLecture} download>
								Descarga la lectura
							</a>
						</div>
						<div className="mb-2">
							<i className="mr-2 fas fa-file-word fa-2x"></i>
							<a href={docxManual} download>
								Descarga el manual del estudiante
							</a>
						</div>
						<div className="mb-2">
				...
				<section className="container d-flex align-items-center justify-content-between p-3">
					{id > 1 ? (
						<Link href={`${baseURL}/liderazgo/clase?id=${id - 1}`}>
							&larr; Anterior
						</Link>
					) : null}
					{id <= clases.length - 1 ? (
						<Link
							className="ml-auto"
							href={`${baseURL}/liderazgo/clase?id=${id + 1}`}
						>
							Próximo &rarr;
						</Link>
					) : null}
