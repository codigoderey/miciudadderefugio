...
const IndexEventos = ({ eventos, user }) => {
  ...
  return (
    <>
      <Head>
        <meta name="description" content="Conozca y registrese para los próximos eventos y actividades de la Iglesia Ciudad de Refugio." />
      ...
      <Layout pageTitle="Conozca los próximos eventos y actividades de la Iglesia Ciudad de Refugio.">
        ...
        <div className="container">
          <section className="">
            <div className="row">
              {eventos.length === 0 ? <p className="mt-3">No hay eventos por el momento.</p> : null}
              {eventos.map(e => (
                        ...
                        <div className="d-flex align-items-center justify-content-between">
                          ...
                          {e.registrados.length >= e.cantidadDisponible
                            ? (<div className="alert alert-danger mt-3">
                              Este evento ha alcanzado su capacidad.</div>)
                            : (<Link href={`/eventos/registrarme/${e._id}`}><a className="thm-btn cta-three__btn mt-0">Registrarme</a></Link>)
                          }
                          {user ? <span className="text-danger hoverable" onClick={() => callToDeleteEvent(e._id)}>Eliminar evento</span> : null}
                        </div>
                      </div>
                    </div>
                  </div>
